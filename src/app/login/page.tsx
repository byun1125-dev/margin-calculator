  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        // 사용자가 팝업을 닫은 경우 - 에러 메시지 없이 조용히 처리
      } else {
              ...
              <button
                onClick={() => {
                  ...
                  setTimeout(() => {
                    ...
                    setError('URL이 복사되었습니다. 크롬 또는 사파리에서 붙여넣기 해주세요.');
                  }, 500);
                }}
                className="w-full py-2.5 border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
              >
                외부 브라우저에서 열기
              </button>
              <p className="text-xs text-gray-400">
                또는 크롬/사파리에서 직접 접속해주세요
              </p>
            </div>
          ) : (
            <button
